...
describe('webhook POST - incoming message triggers welcome + auto form-link send', () => {
  ...
  it('first message of the day sends ONLY welcome+notice combined into one message - the form link is never auto-sent anymore, only via the manual "Enviar formulario" button', async () => {
    const org = await createTestOrg(app.prisma);
    ...
    // Welcome+notice combined into ONE message - no link, no bank-account warning,
    // no follow-up nudge auto-sent anymore. Those three only go out when staff
    // clicks "Enviar formulario" (frontend's own formLinkMessage.ts + POST
    // /inbox/:id/form-link).
    expect(outbound).toHaveLength(1);
    expect(outbound[0].text).toContain('bienvenido');
    expect(outbound[0].text).not.toContain('solo para hacer tu pedido');
    expect(outbound[0].text).not.toContain('Ahorros Bancolombia');
    ...
    // No form link ever generated by the auto-welcome - form_token_min_iat stays
    // null until staff actually clicks "Enviar formulario".
  ...
  it('a SECOND message from the same ticket (even on a later day, isFirstMessageToday again) does NOT repeat the privacy notice - it already got it once, for good - but still gets the welcome message normally', async () => {
    const org = await createTestOrg(app.prisma);
    ...
    const outbound = await app.prisma.ticketMessage.findMany({ where: { ticket_id: ticket.id, direction: 'out' }, orderBy: { sent_at: 'asc' } });
    expect(outbound).toHaveLength(1);
    // La bienvenida sigue mandándose normalmente cada día...
    expect(outbound[0].text).toContain('bienvenido');
