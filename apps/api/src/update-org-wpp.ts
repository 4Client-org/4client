...
async function main() {
  const existing = await prisma.organization.findUniqueOrThrow({ where: { slug: 'fruver-san-gabriel' } });
  const org = await prisma.organization.update({
    ...
    data: {
      ...
      wpp_meta_token: encryptSecret(process.env.META_TOKEN!, existing.id),
    },
  });
