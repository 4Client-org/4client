...
export default async function inboxRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/:ticketId/form-link', { preHandler: [authenticate] }, async (req, reply) => {
    ...
    // link_failed_attempts resets too - this new link gets a clean slate for the
    // per-link wrong-guess count (linkSecurity.ts). The ticket-wide cumulative count
    // (link_failed_total) is NOT reset here on purpose - that's what makes it
    // actually cumulative instead of something a new link resets for free.
